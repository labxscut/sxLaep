...
sxRaep is an efficient machine learning tool for predicting whether a protein sequence is an enzyme or non-enzyme. It combines multi-physicochemical sequence features with an XGBoost classifier for fast and accurate binary classification.

## QUICK LINKS

[GitHub Repository](https://github.com/yourusername/sxraep)
...
sxRaep provides a machine learning-based solution for enzyme/non-enzyme classification from protein sequences. The tool extracts comprehensive sequence-derived features that capture various physicochemical properties and structural characteristics of proteins:
...
sxRaep supports both single-sequence prediction and batch processing of FASTA files, with built-in parallelization for efficient feature extraction. The package provides both a simple Python API and a command-line interface for flexible integration into various bioinformatics workflows.
...
Figure 1. The RAEP pre-screening workflow. The framework acts as a lightweight ”gatekeeper” for massive protein datasets. It extracts alignment-free
...
> sxRaep (Sequence-based Enzyme/Non-Enzyme Prediction)
>
> The package is available for Python 3.7+ on Windows, Linux, and macOS.
>
> Latest updates and source code are available at:
> https://github.com/labxscut/sxRaep


## DOCKER
A pre-configured Docker image is provided to run `sxRaep` in an isolated environment with Python 3.11 and all required dependencies.

### Quick Start
Pull the official image directly:
```bash
docker pull labxscut/sxraep:python3.11
```
### Running the Container
Mount your local data directory to the container's working directory (/workspace) and start an interactive shell:
```bash
docker run --rm -it -v $(pwd):/workspace labxscut/sxraep:python3.11 bash
```
Once inside the container, the raep CLI is ready to use (the Raep_docker conda environment is auto-activated).

### Build Locally (Optional)
If you prefer to build the image from the provided Dockerfile:
```bash
docker build --no-cache -t labxscut/sxraep:python3.11 .
```
...
> (2). # for development #
>
> sxRaep is open source and your contributions are greatly welcome.
>
> First, use git to clone a copy of sxraep:
>
> ```{.python .input}
> user> git clone https://github.com/labxscut/sxRaep
> cd sxraep
> user> pip install -e .
> ```
...
>
> ```{.python .input}
> raep --input sequences.fasta --output predictions.json
> ```
...
>
> ```{.python .input}
> raep                           # main command-line tool for enzyme prediction
>                                # Usage: raep --input <fasta> --output <json>
> ```
...
>
> ```{.python .input}
> raep --input proteins.fasta --output predictions.json
> ```

## NOTES

> - The pre-trained model (`enzyme_xgb_model.pkl`) is included in the package.
> - The Python package name for installation is `raep` (not `sxraep`): `pip install raep`
...
Please cite sxRaep if you use it in your research:

> labxscut. sxRaep: Sequence-based Enzyme/Non-Enzyme Prediction using Sequence Features and XGBoost. (2026).
